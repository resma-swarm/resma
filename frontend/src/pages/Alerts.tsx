...
import { AlertTriangle, Droplets, Waves, AlertOctagon, Search, Filter, ChevronDown, X, ShieldAlert } from "lucide-react"
...
> = {
  oom: { label: "OOM", icon: AlertOctagon, color: "text-destructive", bg: "bg-destructive/15" },
  leak: { label: "Memory Leak", icon: Droplets, color: "text-chart-4", bg: "bg-chart-4/15" },
  drift: { label: "Resource Drift", icon: Waves, color: "text-chart-3", bg: "bg-chart-3/15" },
}
...
export default function Alerts() {
  ...
  // SSE: invalida queries de alerts quando receber evento de dashboard
  // (cluster info) ou metrics (coleta). Os alertas são derivados das métricas,
  // então qualquer coleta nova pode mudar o estado de leaks/drifts.
  ...
  if (alerts.length === 0) {
    return (
      <div className="space-y-6">
        ...
        <Card>
          <EmptyState icon={ShieldAlert} message="Nenhum alerta ativo. Tudo sob controle." />
        </Card>
      </div>
    )
  }
  ...
  const statCards = [
    ...
    { label: "Critical", value: counts.critical, icon: AlertOctagon, iconBg: "bg-destructive/15", iconColor: "text-destructive", valueColor: "text-destructive" },
    { label: "Warning", value: counts.warning, icon: AlertTriangle, iconBg: "bg-warning/15", iconColor: "text-warning", valueColor: "text-warning" },
          ...
          <Table>
            ...
            <TableBody>
              {filteredAlerts.map((alert, i) => {
                const typeCfg = TYPE_CONFIG[alert.type] ?? { label: alert.type, icon: AlertTriangle, color: "text-muted-foreground", bg: "bg-muted" }
                ...
                return (
                  <TableRow
                    ...
                    onClick={() => alert.service && navigate(`/services/${encodeURIComponent(alert.service)}`)}
                  >
                    <TableCell>
                      <div className="flex items-center gap-2">
