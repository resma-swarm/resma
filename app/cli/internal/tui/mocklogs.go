...
var apiLogs = []mockLogEntry{
	...
	{"2026-01-15T14:33:30Z", "DEBUG", `Metrics payload received from agent node-2: {"node_id":"node-2","timestamp":"2026-01-15T14:33:30Z","containers":[{"id":"ctn-abc123","name":"worker-3.1","service":"worker-3","cpu_percent":62.4,"memory_usage":1456000000,"memory_limit":2000000000,"memory_percent":72.8,"network_rx":1024000,"network_tx":512000,"block_read":4096,"block_write":8192,"pids":48},{"id":"ctn-def456","name":"postgres.1","service":"postgres","cpu_percent":15.2,"memory_usage":832000000,"memory_limit":2000000000,"memory_percent":41.6,"network_rx":2048000,"network_tx":1024000,"block_read":16384,"block_write":32768,"pids":32},{"id":"ctn-ghi789","name":"redis-cache.1","service":"redis-cache","cpu_percent":8.1,"memory_usage":280000000,"memory_limit":1000000000,"memory_percent":28.0,"network_rx":512000,"network_tx":256000,"block_read":0,"block_write":0,"pids":12}]}`},
	...
	{"2026-01-15T14:33:35Z", "ERROR", `Panic recovered in HTTP handler: goroutine 42 [running]: main.handleServiceMetrics.func1() /app/internal/server/metrics_handlers.go:87 +0x2a4 main.handleServiceMetrics(0xc0004a0000, 0x7f8e12345678, 0xc0003a0120, 0x10, 0x20) /app/internal/server/metrics_handlers.go:85 +0x1f0 net/http.HandlerFunc.ServeHTTP(0xc0004a0000, 0x7f8e12345678, 0xc0003a0120, 0x10, 0x20) /usr/local/go/src/net/http/server.go:2136 +0x37 net/http.(*ServeMux).ServeHTTP(0xc0004a0000, 0x7f8e12345678, 0xc0003a0120, 0x10, 0x20) /usr/local/go/src/net/http/server.go:2514 +0x1a4 github.com/resma-swarm/resma/app/api/internal/server.withCORS.func1(0xc0004a0000, 0x7f8e12345678, 0xc0003a0120, 0x10, 0x20) /app/internal/server/middleware.go:45 +0x150 net/http.HandlerFunc.ServeHTTP(0xc0004a0000, 0x7f8e12345678, 0xc0003a0120, 0x10, 0x20) /usr/local/go/src/net/http/server.go:2136 +0x37 created by net/http.(*Server).Serve /usr/local/go/src/net/http/server.go:3033 +0x394`},
	{"2026-01-15T14:33:40Z", "WARN", `DuckDB query slow (1.8s): SELECT s.name, s.replicas, s.cpu_limit, s.mem_limit, avg(m.cpu_percent) as avg_cpu, avg(m.mem_percent) as avg_mem, max(m.cpu_percent) as max_cpu, max(m.mem_percent) as max_mem, count(*) as samples FROM services s JOIN metrics m ON s.name = m.service WHERE m.timestamp > now() - interval '1 hour' GROUP BY s.name, s.replicas, s.cpu_limit, s.mem_limit ORDER BY avg_cpu DESC`},
	{"2026-01-15T14:33:45Z", "DEBUG", `SSE event broadcast: event: alerts data: {"type":"alert","level":"critical","service":"batch-processor","message":"OOMKilled: exceeded memory limit of 2Gi","timestamp":"2026-01-15T14:32:50Z","task_id":"task-7","node":"node-2","exit_code":137,"restart_count":3}`},
}
